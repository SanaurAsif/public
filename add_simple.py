...
parser.add_argument("--logout", action="store_true", default=False, help="Logout from all devices (default: False)")
...
while True:
    ...
    async def main():
        async with app:
            ...
            if args.name or args.username:
                ...
                for name_line in names_txt.split("\n"):
                    if len(name_line.split(",")) == 3:
                        ...
                        if not f_name.isalnum():
                            continue
                        if not l_name.isalnum():
                            continue
                        found_uni = True
                        with open(args.file, "w") as names_file:
                            names_file.write(names_txt.replace(f_name + "," + l_name, "used"))
                            names_file.close()
                        break
            ...
            if args.logout:
                try:
                    logo = await app.invoke(ResetAuthorizations())
                    print(logo)
